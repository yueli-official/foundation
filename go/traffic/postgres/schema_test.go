...
func TestSchemaV1ContainsTrafficTruth(t *testing.T) {
	...
	for _, table := range []string{
		"traffic_instances",
		"traffic_event_receipts",
		"traffic_totals",
		"traffic_daily",
		"traffic_visitor_markers",
		"traffic_baselines",
	} {
	...
	if !strings.Contains(migration.DownSQL, "DROP TABLE IF EXISTS traffic_instances") {
		t.Fatal("down schema does not remove traffic_instances")
	}
...
func TestWriteMigrationIsImmutable(t *testing.T) {
	directory := t.TempDir()
	written, err := postgres.WriteMigration(directory, "0014_traffic_v1", 1)
	if err != nil {
		t.Fatal(err)
	}
	...
	if !strings.Contains(string(up), "-- traffic-schema-version: 1") ||
		!strings.Contains(string(up), "-- traffic-schema-digest: ") {
		t.Fatalf("missing generated metadata: %q", up[:min(len(up), 180)])
	}
	repeated, err := postgres.WriteMigration(directory, "0014_traffic_v1", 1)
	if err != nil {
		t.Fatal(err)
	}
	if repeated != written {
	...
	if _, err := postgres.WriteMigration(directory, "0014_traffic_v1", 1); err == nil {
		t.Fatal("migration drift was overwritten")
	}
	if filepath.Base(written.DownPath) != "0014_traffic_v1.down.sql" {
