...
async function archiveFiles(file: File): Promise<ImportFile[]> {
  ...
  const entries = await new Promise<Record<string, Uint8Array>>((resolve, reject) => {
    void file.arrayBuffer().then(buffer => unzip(new Uint8Array(buffer), {
      filter(entry) {
        total += entry.originalSize;
        if (++count > 500 || total > MAX_EXPANDED || entry.originalSize > MAX_FILE) {
          failure = "压缩包过大，请拆分后导入";
          return false;
        }
        return !entry.name.endsWith("/");
      },
    }, (error, data) => error ? reject(error) : resolve(data))).catch(reject);
  });
...
export async function readImportDocument(files: File[]): Promise<ImportDocument> {
  if (!files.length) throw new Error("请选择要导入的文档");
  const entries: ImportFile[] = [];
  for (const file of files) {
    if (file.size > MAX_FILE) throw new Error(`${file.name} 超过 50 MB，请拆分后导入`);
    if (/\.zip$/i.test(file.name)) entries.push(...await archiveFiles(file));
    else entries.push({ path: importPath(file.webkitRelativePath || file.name), file });
  }
  const documents = entries.filter(entry => /\.(md|markdown|html?|docx)$/i.test(entry.path));
  if (documents.length !== 1) throw new Error("请一次选择一份 Markdown、HTML 或 Word 文档，以及它引用的图片");
  ...
  if (/\.docx$/i.test(document.path)) {
    ...
    const result = await mammoth.convertToHtml({ arrayBuffer: await document.file.arrayBuffer() });
    return { name: document.path, format: "html", content: result.value, files: entries };
  }
  return {
    name: document.path,
    format: /\.html?$/i.test(document.path) ? "html" : "markdown",
    content: await document.file.text(), files: entries,
  };
}
...
export function findImportImage(src: string, documentName: string, files: ImportFile[]): File | undefined {
...
export async function resolveImportImage(src: string, document: ImportDocument): Promise<File> {
  ...
  if (/^data:image\//i.test(src)) {
    ...
    return imageFile(new File([blob], `imported-image.${ext}`, { type: blob.type }));
  }
  if (/^https?:\/\//i.test(src)) {
    try {
      const response = await fetch(src, { credentials: "omit", signal: AbortSignal.timeout(30000) });
      if (!response.ok) throw new Error();
      if (Number(response.headers.get("content-length")) > MAX_FILE) throw new Error();
      const blob = await response.blob();
      if (blob.size > MAX_FILE || !blob.type.startsWith("image/")) throw new Error();
      return imageFile(new File([blob], new URL(src).pathname.split("/").pop() || "image.png", { type: blob.type }));
    } catch { throw new Error(`无法读取图片：${src}。请下载图片后，与文档一起选择再导入。`); }
  }
...
export async function uploadImportImages(
  ...
  if (images.length && !uploader) throw new Error("当前编辑器尚未接入图片上传，无法导入带图文档");
  const sources = [...new Set(images.map(node => String(node.attrs?.src || "")))];
  ...
  for (const src of sources) {
    ...
    if (!cache.has(src)) {
      try { resolved.set(src, await resolveImportImage(src, document)); }
      catch (error) {
  ...
  for (const [src, file] of resolved) {
    ...
    try { cache.set(src, await uploader!(file)); }
    catch (error) {
      if (!/^https?:\/\//i.test(src)) throw error;
      failed.add(src);
    }
    ...
    parent.content = parent.content?.flatMap((node): JSONContent[] => {
      ...
      if (node.type === "image" && failed.has(src)) {
        const content: JSONContent[] = [
          { type: "text", text: `［图片未导入：${String(node.attrs?.alt || "外链图片")} · ` },
          { type: "text", text: "原图地址", marks: [{ type: "link", attrs: { href: src } }] },
          { type: "text", text: "］" },
        ];
        return parent.type === "paragraph" || parent.type === "heading" ? content : [{ type: "paragraph", content }];
      }
      if (node.type === "image") node.attrs = { ...node.attrs, src: cache.get(src) };
