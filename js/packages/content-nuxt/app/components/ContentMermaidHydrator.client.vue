...
async function renderMermaid() {
  ...
  target.querySelectorAll<HTMLElement>('.mermaid-diagram[data-src]').forEach((div) => {
    targets.push({ host: div, src: decodeURIComponent(div.dataset.src || '') })
  })
  ...
  for (const { host, src } of targets) {
    ...
    try {
      ...
      // Keep the source block visible when rendering fails.
    }
  }
}

watch(() => props.target, () => nextTick(renderMermaid), { immediate: true })
watch(() => colorMode.value, () => nextTick(renderMermaid))
</script>
