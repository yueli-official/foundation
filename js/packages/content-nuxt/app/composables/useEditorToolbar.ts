...
export const useEditorToolbar = () => {
  ...
  const toolbarItems = computed<EditorToolbarItem[][]>(() => [
    [
      { kind: 'undo', icon: 'i-tabler-arrow-back-up', tooltip: { text: '撤销' } },
      { kind: 'redo', icon: 'i-tabler-arrow-forward-up', tooltip: { text: '重做' } },
    ],
    [
      {
      ...
      },
      {
        ...
        items: [
          ...
          { kind: 'orderedList', icon: 'i-tabler-list-numbers', label: '有序列表' },
        ],
      },
      { kind: 'blockquote', icon: 'i-tabler-blockquote', tooltip: { text: '引用' } },
      { kind: 'codeBlock', icon: 'i-tabler-code-dots', tooltip: { text: '代码块' } },
    ],
    [
      { kind: 'mark', mark: 'bold', icon: 'i-tabler-bold', tooltip: { text: '粗体' } },
      { kind: 'mark', mark: 'italic', icon: 'i-tabler-italic', tooltip: { text: '斜体' } },
      { kind: 'mark', mark: 'underline', icon: 'i-tabler-underline', tooltip: { text: '下划线' } },
      { kind: 'mark', mark: 'strike', icon: 'i-tabler-strikethrough', tooltip: { text: '删除线' } },
      { kind: 'mark', mark: 'code', icon: 'i-tabler-code', tooltip: { text: '行内代码' } },
    ],
    [
      { kind: 'link', icon: 'i-tabler-link', tooltip: { text: '链接' } },
      { kind: 'image', icon: 'i-tabler-photo', tooltip: { text: '图片' } },
      { kind: 'horizontalRule', icon: 'i-tabler-separator', tooltip: { text: '分隔线' } },
    ],
    [
      {
        ...
        items: [
          ...
          { kind: 'callout-important', icon: 'i-tabler-alert-circle', label: '重要' },
          { kind: 'callout-warning', icon: 'i-tabler-alert-triangle', label: '警告' },
      ...
      },
      {
        ...
        items: [
          ...
          { kind: 'math-block', icon: 'i-tabler-math-function', label: '公式块' },
        ],
      },
      { kind: 'mermaid-block', icon: 'i-tabler-chart-dots-3', tooltip: { text: 'Mermaid 图表' } },
    ],
    [
      {
        ...
        content: { align: 'end' },
        items: [
          { kind: 'textAlign', align: 'left', icon: 'i-tabler-align-left', label: '左对齐' },
          { kind: 'textAlign', align: 'center', icon: 'i-tabler-align-center', label: '居中' },
          { kind: 'textAlign', align: 'right', icon: 'i-tabler-align-right', label: '右对齐' },
          { kind: 'textAlign', align: 'justify', icon: 'i-tabler-align-justified', label: '两端对齐' },
        ],
      },
    ],
    [
      { kind: 'clearFormatting', icon: 'i-tabler-clear-formatting', tooltip: { text: '清除格式' } },
    ],
  ])
  ...
  const suggestionItems = computed<EditorSuggestionMenuItem[][]>(() => [
    ...
    [
      ...
      { kind: 'horizontalRule', label: '分隔线', icon: 'i-tabler-separator-horizontal' },
      ...
      { kind: 'mermaid-block', label: 'Mermaid 图表', icon: 'i-tabler-chart-dots-3' },
    ],
  ])

  const imageBubbleItems = computed<EditorToolbarItem[][]>(() => [
    [
      { kind: 'download-image', icon: 'i-tabler-download', tooltip: { text: '下载图片' } },
      { kind: 'remove-image', icon: 'i-tabler-trash', tooltip: { text: '删除图片' } },
    ],
  ])
  ...
  const dragHandleItems = (editor: Editor): DropdownMenuItem[][] => {
    ...
    return mapEditorItems(
      ...
      [
        ...
        [
          { kind: 'duplicate', pos: selectedNode.value?.pos, label: '复制块', icon: 'i-tabler-copy' },
          {
            label: '复制到剪贴板',
        ...
        [
          { kind: 'moveUp', pos: selectedNode.value?.pos, label: '上移', icon: 'i-tabler-arrow-up' },
          { kind: 'moveDown', pos: selectedNode.value?.pos, label: '下移', icon: 'i-tabler-arrow-down' },
        ],
        [
          { kind: 'delete', pos: selectedNode.value?.pos, label: '删除', icon: 'i-tabler-trash' },
        ],
      ],
      {},
    ) as DropdownMenuItem[][]
