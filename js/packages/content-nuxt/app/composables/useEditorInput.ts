...
import {
  cleanImportHTML, collectImportImages, imageFile, readImportDocument, uploadImportImages,
  type ImportDocument,
...
export function useEditorInput(
  ...
  function insertion(position?: number) {
    ...
    if (position !== undefined) {
      ...
      bookmark = selectionType.near(editor.state.doc.resolve(position)).getBookmark();
    }
    ...
    return {
      insert(content: JSONContent[]) {
        if (editor.isDestroyed) throw new Error("编辑器已关闭，请重新打开后导入");
    ...
    try {
      await queue(async () => {
        ...
        const document: ImportDocument = html ? {
          name: "clipboard.html", format: "html", content: html,
          files: files.map(file => ({ path: file.name, file })),
        } : await readImportDocument(files);
        const identity = document.name + document.content;
        if (identity !== importIdentity) { importCache = new Map(); importIdentity = identity; }
        const editor = editorRef.value?.editor;
        if (!editor) throw new Error("编辑器尚未准备好");
        let tree: JSONContent;
        if (document.format === "markdown") {
          const markdown = (editor as unknown as { markdown?: { parse: (text: string) => JSONContent } }).markdown;
          if (!markdown) throw new Error("Markdown 解析器尚未准备好");
          tree = markdown.parse(document.content);
        } else {
          const html = new DOMParser().parseFromString(await cleanImportHTML(document.content), "text/html");
          ...
          tree = generateJSON(html.body.innerHTML, editor.extensionManager.extensions as never);
        ...
        const uploaded = await uploadImportImages(tree, document, uploader(), text => { progress.value = text; }, importCache);
        if (!uploaded.content?.length) throw new Error("文档中没有可导入的正文");
        point.insert(uploaded.content);
        importCache = new Map(); importIdentity = "";
      });
    } finally { point.release(); }
  }
  ...
  function paste(event: ClipboardEvent) {
    ...
    void uploadImages(files).catch(error => reportError(error instanceof Error ? error.message : "图片上传失败"));
  }
  function drop(event: DragEvent) {
    ...
    void uploadImages(files, "", position).catch(error => reportError(error instanceof Error ? error.message : "图片上传失败"));
  }
  function dragOver(event: DragEvent) {
    if (uploader() && Array.from(event.dataTransfer?.types || []).includes("Files")) event.preventDefault();
  }
  return { imageOpen, importOpen, uploading, progress, uploadImages, importDocument, paste, drop, dragOver };
}
