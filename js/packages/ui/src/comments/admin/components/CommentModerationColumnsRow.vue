...
const props = defineProps<{ comment: CommentModerationItem; formatDate: (value: string) => string }>();
...
const menuItems = computed(() => {
  ...
  const groups: (readonly AdminRowActionItem[])[] = Array.isArray(items[0])
    ...
    : items.length ? [items as readonly AdminRowActionItem[]] : [];
  if (props.comment.approve) groups.unshift([{
    id: "approve", label: "通过审核", icon: "i-tabler-check",
...
<template>
  <div class="comment-columns min-w-0 py-1" data-manage-comment-row data-comment-moderation-row data-comment-columns-row>
    <div class="col-span-3 flex min-w-0 items-start gap-3 lg:col-span-1">
      ...
      <div class="min-w-0 flex-1">
        <div class="flex min-w-0 flex-wrap items-baseline gap-x-2 gap-y-1">
          <span class="truncate text-sm font-semibold text-highlighted">{{ comment.authorName }}</span>
        ...
        <div v-if="comment.reply" class="mt-2 border-l border-default pl-2 text-xs leading-5 text-muted" data-comment-reply-context>
        ...
        <p class="mt-1 whitespace-pre-wrap text-sm leading-6 text-default [overflow-wrap:anywhere]" data-comment-body>{{ comment.content }}</p>
        <details v-if="comment.authorEmail" class="mt-1 text-xs text-muted" data-comment-user-details>
          <summary class="w-fit cursor-pointer hover:text-primary">用户详情</summary>
          <p class="break-all py-1">邮箱：{{ comment.authorEmail }}</p>
        </details>
      </div>
    </div>
    <div class="min-w-0 pl-10 text-xs text-muted lg:pl-0" data-comment-source>
      <NuxtLink v-if="comment.source.to" :to="comment.source.to" class="flex min-w-0 items-center gap-1.5 hover:text-primary" :title="comment.source.label">
        <UIcon :name="comment.source.icon" class="size-3.5 shrink-0" /><span class="truncate">{{ comment.source.label }}</span>
      </NuxtLink>
      <span v-else class="block truncate" :title="comment.source.label">{{ comment.source.label }}</span>
    </div>
    <div>
    ...
    <div class="flex justify-end">
      <AdminRowActions v-if="menuItems.length" :items="menuItems" :label="`评论操作：${comment.authorName}`" presentation="overflow" overflow-icon="i-tabler-dots" class="[--yueli-row-action-size:2rem]" />
    </div>
  </div>
</template>
