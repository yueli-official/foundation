...
<template>
  <div
    class="flex min-w-0 items-start gap-4 py-1"
    data-manage-comment-row
    data-comment-moderation-row
    data-comment-compact-row
  >
    <UAvatar
    ...
    <div class="min-w-0 flex-1 space-y-2">
      <div class="flex min-w-0 flex-wrap items-baseline gap-x-2 gap-y-1">
        <span
          class="max-w-full truncate text-sm font-semibold text-highlighted"
      ...
      <div
        v-if="comment.reply"
        class="max-w-prose border-l border-default pl-2 text-xs leading-5 text-muted"
      ...
      <p
        class="max-w-prose whitespace-pre-wrap break-words text-sm leading-6 text-default [overflow-wrap:anywhere]"
        data-comment-body
      >
        {{ comment.content }}
      </p>
      <div
        class="flex min-w-0 flex-wrap items-center gap-x-3 gap-y-1 text-xs text-muted"
        data-comment-actions
      >
        <NuxtLink
          v-if="comment.source.to"
          :to="comment.source.to"
          class="inline-flex min-w-0 max-w-full items-center gap-1 hover:text-primary"
        >
        ...
        >
          <summary class="w-fit cursor-pointer text-dimmed hover:text-primary">
            用户详情
          </summary>
    ...
    <NuxtLink
      v-if="comment.source.thumbnailUrl && comment.source.to"
      :to="comment.source.to"
      :aria-label="`查看图片：${comment.source.label}`"
      class="hidden shrink-0 self-center overflow-hidden rounded-md sm:block"
      data-comment-thumbnail
    >
      <img
        :src="comment.source.thumbnailUrl"
        alt=""
        width="80"
        height="60"
        loading="lazy"
        class="h-[60px] w-20 object-cover"
      />
    </NuxtLink>
  </div>
</template>
