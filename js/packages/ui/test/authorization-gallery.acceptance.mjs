...
import {
  chromium,
  expect,
} from "../../../../../gallery/web/node_modules/@playwright/test/index.mjs";
import { mkdir, writeFile } from "node:fs/promises";
const origin = process.env.GALLERY_E2E_URL || "http://192.168.5.7:3007";
const account =
  process.env.GALLERY_E2E_ACCOUNT_URL || "http://192.168.5.7:3400";
const output =
  process.env.ACCEPTANCE_OUTPUT || "E:/tmp/yueli-authorization-shared";
await mkdir(output, { recursive: true });
const browser = await chromium.launch();
const report = {
  fixtureApplications: true,
  fixtureReviews: true,
  realIdentity: false,
};
try {
  const context = await browser.newContext();
  const login = await context.request.post(`${account}/api/v1/auth/login`, {
    data: {
      email: process.env.E2E_EMAIL || "test@example.com",
      password: process.env.E2E_PASSWORD || "Yueli-local-development-2026",
    },
  });
  ...
  let applications = ["approve", "reject"].map((id) => ({
    id: `fixture-${id}`,
    subject: "TestA123",
    role: "content_operator",
    reason: "统一组件验收：申请参与内容维护。",
    createdAt: "2026-09-08T01:02:00Z",
  }));
  ...
  await page.route("**/manage/applications/fixture-*/review", async (route) => {
    ...
    applications = applications.filter(
      (item) => !route.request().url().includes(item.id),
    );
    await route.fulfill({
      status: 200,
  ...
  await page.goto(`${origin}/manage/authorization`, {
    waitUntil: "networkidle",
  });
  const rows = page.locator("[data-authorization-application]");
  await expect(rows).toHaveCount(2);
  await expect(rows.first()).toContainText("测试管理员");
  report.realIdentity = true;
  ...
  for (const width of [1440, 390]) {
    ...
    await page.screenshot({
      path: `${output}/applications-${width}.png`,
      fullPage: true,
    });
  }
  ...
  const [profile] = await Promise.all([
    ...
    profileLink.locator("span").click(),
  ]);
  await profile.waitForLoadState("networkidle");
  await expect(profile.locator("body")).toContainText("测试管理员");
  await profile.close();
  await rows.first().getByRole("button", { name: "批准", exact: true }).click();
  await expect(
    rows.first().getByRole("button", { name: "拒绝", exact: true }),
  ).toBeDisabled();
  ...
  await expect(
    page
      .getByRole("region", { name: "已授权用户" })
      .getByText("测试管理员", { exact: true }),
  ).toBeVisible();
  ...
  await page.reload({ waitUntil: "networkidle" });
  await expect(
    page.getByText("用户资料加载失败", { exact: true }),
  ).toBeVisible();
  fail = false;
  ...
  await expect(page.getByText("用户资料加载失败", { exact: true })).toHaveCount(
    0,
  );
