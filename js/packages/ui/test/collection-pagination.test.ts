...
const PaginationStub = defineComponent({
  props: ["page", "total", "itemsPerPage", "showEdges", "siblingCount"],
  emits: ["update:page"],
  template: "<div />",
});
...
describe("shared pagination", () => {
  it("retains bounded page navigation and folds long ranges", () => {
    const wrapper = mount(CollectionPagination, {
      props: { modelValue: 50, totalPages: 100, compact: true },
      global: { components: { UPagination: PaginationStub } },
    });
  ...
  it("accepts only declared page sizes and omits result summaries", () => {
    const wrapper = mount(CollectionPaginationBar, {
      props: {
        page: 1,
        pageSize: 20,
        total: 105,
        pageSizes: [20, 40, 60],
        pageSizeOption: (value: number) => `${value}/页`,
      },
      global: {
        components: { USelect: SelectStub, UPagination: PaginationStub },
      },
    });
