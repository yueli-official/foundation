...
const passthrough = (name: string, tag = "div") => defineComponent({
  name,
  inheritAttrs: false,
  props: { text: String, to: String, target: String, rel: String, items: Array },
...
const global = {
  components: {
    ...
    UIcon: defineComponent({
      name: "UIcon",
      props: { name: String },
      setup: (props, { attrs }) => () => h("span", { ...attrs, "data-icon": props.name }),
    }),
...
describe("AdminRowActions", () => {
  it("owns the compact size, glyph size, centering and accessible names", async () => {
    ...
    const wrapper = mount(AdminRowActions, {
      props: {
        label: "文章操作",
        items: [
          { id: "view", label: "查看文章", icon: "i-tabler-external-link", to: "/posts/one" },
          { id: "edit", label: "编辑文章", icon: "i-tabler-pencil", onSelect: edit },
        ],
      },
      global,
    });
    ...
    for (const action of actions) {
      expect(action.classes()).toEqual(expect.arrayContaining(["size-11", "sm:size-6", "items-center", "justify-center", "p-0"]));
      expect(action.get("[data-icon]").classes()).toEqual(expect.arrayContaining(["block", "size-4"]));
  ...
  it("owns the vertical overflow trigger and preserves menu groups", () => {
    const wrapper = mount(AdminRowActions, {
      props: {
        ...
        items: [
          ...
          [{ id: "delete", label: "删除", icon: "i-tabler-trash", tone: "danger" }],
        ],
      },
      global,
    });
    ...
    expect(wrapper.getComponent({ name: "UDropdownMenu" }).props("items")).toHaveLength(2);
  });
});
