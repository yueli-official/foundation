...
describe("collection editing gesture", () => {
  it.each(["button", "a", "input", "select", "textarea", "summary", '[role="checkbox"]', '[contenteditable="true"]'])("does not steal interaction from %s or its descendants", (selector) => {
    ...
    if (selector.startsWith("[contenteditable")) control.setAttribute("contenteditable", "true");
    const icon = document.createElement("span");
    control.append(icon); parent.append(control);
    let edits = 0;
    parent.addEventListener("dblclick", event => { if (isCollectionEditGesture(event)) edits++; });
  ...
  it("allows content but respects prevented gestures", () => {
    ...
    content.addEventListener("dblclick", event => results.push(isCollectionEditGesture(event)));
